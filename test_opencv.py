...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        # self.test_on_off_camera = tk.Button(self.camera_tap1, text="Test on/off cam", command=self.on_off_vid)
        # self.test_on_off_camera.pack()
        ...
        #----------------------------------- load image -----------------------------------#
        # self.stop_img = tk.PhotoImage(file = "F:\\Program PLC\\Solar_thermal_Project\\haliogen_project\\Photo\\stop.png")
        # self.up_img = tk.PhotoImage(file = "F:\\Program PLC\\Solar_thermal_Project\\haliogen_project\\Photo\\up-arrow.png")
        # self.down_img = tk.PhotoImage(file = "F:\\Program PLC\\Solar_thermal_Project\\haliogen_project\\Photo\\download.png")
        # self.forward_img = tk.PhotoImage(file = "F:\\Program PLC\\Solar_thermal_Project\\haliogen_project\\Photo\\forward.png")
        # self.reverse_img = tk.PhotoImage(file = "F:\\Program PLC\\Solar_thermal_Project\\haliogen_project\\Photo\\reverse.png")
        # self.top_left_img = tk.PhotoImage(file = "F:\\Program PLC\\Solar_thermal_Project\\haliogen_project\\Photo\\top_left.png")
        # self.top_right_img = tk.PhotoImage(file = "F:\\Program PLC\\Solar_thermal_Project\\haliogen_project\\Photo\\top_right.png")
        # self.bottom_left_img = tk.PhotoImage(file = "F:\\Program PLC\\Solar_thermal_Project\\haliogen_project\\Photo\\bottom_left.png")
        # self.bottom_right_img = tk.PhotoImage(file = "F:\\Program PLC\\Solar_thermal_Project\\haliogen_project\\Photo\\bottom_right.png")
...
#--------------------------------------------------------------------------------------------------------------------------------------------------#
    ...
    def click_event(self, event):
        ...
        # print(self.roi[event.y, event.x])
        # self.points.append([event.x, event.y]) #store x, y
    ...
    def current_page(self):
        ...
        # self.after(500, self.current_page)        
    ...
    def open_text_file(self):
        ...
        try:
            ...
            #0:600 = 8:00-8:59:54
            #600:1200 = 9:00-9:59:54
            # for i in f.readlines()[4800:5400] :
    ...
    def enabal_button(self):
        ...
        self.send_point.config(state=tk.NORMAL)
    #--------------------------run path---------------------------#
    ...
    def offset_azimuth(self, dir, off_set, speed):
        ...
        # self.move_manual_stepper(self.motor_x1)
    ...
    def show_frame(self):
        ...
        if self.open_vid:
            ...
            # if not self.state:
            #     #Tracking Calibarte
            #     if ((time.time() - self.last_time_tracking > 60) and self.state_change):
            #         print("Tracking calibarte done status {}".format(self.state_change))
            #         conn = self.get_client(self.clicked.get())
            #         payload = {'state': 'stop' }
            #         payload = f'{payload}\n'
            #         # conn.sendall(payload.encode('utf-8'))
            ...
            #         self.last_time_tracking = time.time()
            #         self.time_delay_tracking = time.time()
            #         self.state_change = False
                
            #         self.motor_y1["dir"] = 0
            #         self.motor_y1["pwm"] = 40
            #         self.motor_y1['speed'] = 200
            #         # self.move_manual_stepper(self.motor_y1)
            #         #time.sleep(0.1)
            #         if "07:00:00" < datetime.now().strftime("%H:%M:%S") < "07:29:00": self.offset_azimuth(0, 27, speed=(27/0.1))
            #         if "07:29:00" < datetime.now().strftime("%H:%M:%S") < "08:29:00": self.offset_azimuth(0, 20, speed=(20/0.1))
            #         if "08:29:00" < datetime.now().strftime("%H:%M:%S") < "08:59:00": self.offset_azimuth(0, 15, speed=(15/0.1))
            #         if "08:59:00" < datetime.now().strftime("%H:%M:%S") < "09:45:00": self.offset_azimuth(0, 11, speed=(11/0.1))
            #         if "09:45:00" < datetime.now().strftime("%H:%M:%S") < "10:45:00": self.offset_azimuth(0, 5, speed=(5/0.1))
            #         if "14:34:00" < datetime.now().strftime("%H:%M:%S") < "14:59:00": self.offset_azimuth(1, 5, speed=(5/0.1))
            #         if "14:59:00" < datetime.now().strftime("%H:%M:%S") < "15:29:00": self.offset_azimuth(1, 7, speed=(7/0.1))
            #         if "15:29:00" < datetime.now().strftime("%H:%M:%S") < "15:45:00": self.offset_azimuth(1, 9, speed=(9/0.1))
            #         if "15:45:00" < datetime.now().strftime("%H:%M:%S") < "16:30:00": self.offset_azimuth(1, 14, speed=(14/0.1))
                
            #     #Tracking Receiver
            #     if ((time.time() - self.last_time_tracking > 60) and not self.state_change):
            #         print("Tracking receiver done status {}".format(self.state_change))
            #         conn = self.get_client(self.clicked.get())
            #         payload = {'state': 'stop' }
            #         payload = f'{payload}\n'
            #         # conn.sendall(payload.encode('utf-8'))
            ...
            #         self.motor_y1['speed'] = 200
            #         # self.move_manual_stepper(self.motor_y1)
            #         if "07:00:00" < datetime.now().strftime("%H:%M:%S") < "07:29:00": self.offset_azimuth(1, 27, (27/0.1))
            #         if "07:29:00" < datetime.now().strftime("%H:%M:%S") < "08:29:00": self.offset_azimuth(1, 20, (20/0.1))
            #         if "08:29:00" < datetime.now().strftime("%H:%M:%S") < "08:59:00": self.offset_azimuth(1, 15, (15/0.1))
            #         if "08:59:00" < datetime.now().strftime("%H:%M:%S") < "09:45:00": self.offset_azimuth(1, 11, (11/0.1))
            #         if "09:45:00" < datetime.now().strftime("%H:%M:%S") < "10:45:00": self.offset_azimuth(1, 5, (5/0.1))
            #         if "14:34:00" < datetime.now().strftime("%H:%M:%S") < "14:59:00": self.offset_azimuth(0, 5, (5/0.1))
            #         if "14:59:00" < datetime.now().strftime("%H:%M:%S") < "15:29:00": self.offset_azimuth(0, 7, (7/0.1))
            #         if "15:29:00" < datetime.now().strftime("%H:%M:%S") < "15:45:00": self.offset_azimuth(0, 9, (9/0.1))
            #         if "15:45:00" < datetime.now().strftime("%H:%M:%S") < "16:30:00": self.offset_azimuth(0, 14, (14/0.1))
            #         time.sleep(0.1)
                    
            #     if self.state_change:
            #         if (time.time() - self.time_delay_tracking > 15):
            #             #print("Start tracking cal")
            #             self.get_contours(sorted_calibarte_contours, self.roi, 320, 240, self.path_data_log1, True, 0)
            #     else:
            #         if (time.time() - self.time_delay_tracking > 15):
